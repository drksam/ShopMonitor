"""
Web routes for NooyenUSATracker integration
...
from .nooyen_integration import integration, get_last_sync_time
...
@login_required
def config_page():
    """Configuration page for NooyenUSATracker integration"""
...
def save_config():
    ...
    # Save configuration
    if api_url:
        os.environ['NOOYEN_API_BASE_URL'] = api_url
        current_app.config['NOOYEN_API_BASE_URL'] = api_url
    
    if api_key:
        os.environ['NOOYEN_API_KEY'] = api_key
        current_app.config['NOOYEN_API_KEY'] = api_key
    ...
    if api_url and (api_key or current_app.config.get('NOOYEN_API_KEY')):
        if integration.app:
...
@login_required
def api_docs():
    """API documentation page for NooyenUSATracker integration"""
    # Check if user is admin
    if not current_user.role or current_user.role.name.lower() != 'admin':
        flash('You do not have permission to access this page.', 'danger')
        return redirect(url_for('dashboard'))
...
@login_required
def available_users():
    """Display available users from NooyenUSATracker"""
    # Check if user is admin
    if not current_user.role or current_user.role.name.lower() != 'admin':
        flash('You do not have permission to access this page.', 'danger')
        return redirect(url_for('dashboard'))
    ...
    if not api_url or not has_api_key:
        flash('NooyenUSATracker integration is not configured. Please configure it first.', 'warning')
        return redirect(url_for('integration_web.config_page'))
...
def manage_user_permissions(user_id):
    """Manage user permissions with NooyenUSATracker integration"""
    # Check if user is admin
    if not current_user.role or current_user.role.name.lower() != 'admin':
        flash('You do not have permission to access this page.', 'danger')
        return redirect(url_for('dashboard'))
